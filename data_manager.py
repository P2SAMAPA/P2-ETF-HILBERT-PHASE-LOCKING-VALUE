"""
data_manager.py  —  HuggingFace data loader for the Hilbert PLV engine
=======================================================================
...
- Log returns  : computed in hilbert_plv.py as  log(price_t / price_{t-1})
- Macro columns: raw levels; pct_change() applied inside hilbert_plv.py
"""
...
def validate_data(prices: pd.DataFrame, macro: pd.DataFrame) -> None:
    ...
    for ticker in list(prices.columns)[:3]:
        ...
        if len(col) > 10 and abs(col.median()) < 0.05:
            logger.warning(
                ...
                "not prices! Log-return computation in hilbert_plv.py will be wrong."
            )
    ...
    if len(prices) < 252:
        logger.warning(
            ...
            "Longer PLV windows (252d+) will return NaN."
        )
